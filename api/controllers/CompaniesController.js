...
module.exports = {
    ...
    notes: function(req, res) {
        if (typeof(req.params.id) != 'undefined' && !isNaN(parseInt(req.params.id))) {
            Database.dataSproc('GetCompanyNotes', [
    ...
    deletecompany: function(req, res) {
        ...
        if (req.session.user.policy[req.route.path].delete == 1) { // Delete
								    // access
            // on
            // this
            // route?
    ...
    export: function(req, res) {
        ...
        sails.controllers.companies.querycompanies(req, res, function(responseCompanies) {
            ...
            AWS.config.update({
                "accessKeyId": "AKIAJ7AKNL3ASNPN3IBA",
                "secretAccessKey": "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",
                "region": "us-west-2"
            });
    ...
    querycompanies: function(req, res, cb) {
        ...
        Database.dataSproc('SearchCompanies', [company_search, req.query.start, req.query.length, orderstring
                                               ,Security.resourceAccess(req,'/admin/users/:id?',{create:1,read:1,update:1,delete:1}),req.session.user.id
            ...
            function loopNotes(i) {
                if (notes[i].id.toString().indexOf('new') > -1) { // new
                    ...
                    Database.dataSproc('CreateNote', [notes[i].note, req.session.user.username, 'NOW()', tempOutNoteVar], function(err, responseNote) {
                        ...
                        Database.dataSproc('CreateNoteMapping', [company.company_id, responseNote[1][tempOutNoteVar], 2, '@outId'], function(err, responseNoteMapping) {
                            if (err)
                                return res.json({
                                    error: 'Database Error:' + err
                                }, 500);
