...
BEGIN
	SELECT cfg_tag_id.id as 'tag_id'
		  ...
		  ,(CASE WHEN cfg_tag_id.alarm_type IS NOT NULL AND cur_tags.value = 1 
			THEN ref_alarms_def.status 
			ELSE cur_tags.value END) 'status_value'  #This funky logic is for E-Stop as they are not Graphics Tags
...
LEFT JOIN ref_alarms_def ON (ref_alarms_def.id = cfg_tag_id.alarm_type)
		INNER JOIN ref_status_def ON (ref_status_def.id = (CASE WHEN cfg_tag_id.alarm_type IS NOT NULL AND cur_tags.value = 1 THEN ref_alarms_def.status ELSE cur_tags.value END))
		...
		WHERE cfg_tag_id.ctrl_type IS NULL
			AND ((cfg_tag_id.alarm_type IS NULL AND cfg_tag_id.count_ID IS NULL) 
				OR (cfg_tag_id.alarm_type = 13));
