...
BEGIN
	SELECT
		SQL_CALC_FOUND_ROWS cur_airplanes.id AS 'airplane_id'
	...
	WHERE 
		...
		AND (airplaneSearchTerms IS NULL OR MATCH (cur_airplanes.resource_category,cur_airplanes.fixed_wing_type,
				cur_airplanes.call_sign,cur_airplanes.serial_number, cur_airplanes.seats) AGAINST (airplaneSearchTerms IN BOOLEAN MODE))
