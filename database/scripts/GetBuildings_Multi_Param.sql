...
WHERE
	...
	AND (CASE WHEN capRateMax IS NOT NULL THEN (cur_buildings.cap_rate IS NULL OR cur_buildings.cap_rate <= capRateMax) ELSE 1 END)
	AND (CASE WHEN capRateMin IS NOT NULL THEN (cur_buildings.cap_rate >= capRateMin) ELSE 1 END)
	AND (CASE WHEN elevatorInstalledYearMax IS NOT NULL THEN (cur_buildings.elevator_installed_year IS NULL OR cur_buildings.elevator_installed_year <= elevatorInstalledYearMax) ELSE 1 END)
	...
	AND (CASE WHEN lastElevatorInstalledYearMax IS NOT NULL THEN (cur_buildings.last_elevator_upgrade_year IS NULL OR cur_buildings.last_elevator_upgrade_year <= lastElevatorInstalledYearMax) ELSE 1 END)
	...
	AND (CASE WHEN lastBoilerInstalledYearMax IS NOT NULL THEN (cur_buildings.last_boiler_upgrade_year IS NULL OR cur_buildings.last_boiler_upgrade_year <= lastBoilerInstalledYearMax) ELSE 1 END)
	...
	AND (CASE WHEN heatAgeMax IS NOT NULL THEN (cur_buildings.heat_system_age IS NULL OR cur_buildings.heat_system_age <= heatAgeMax) ELSE 1 END)
	AND (CASE WHEN heatAgeMin IS NOT NULL THEN (cur_buildings.heat_system_age >= heatAgeMin) ELSE 1 END)
	AND (CASE WHEN boilerAgeMax IS NOT NULL THEN (cur_buildings.boiler_installed_year IS NULL OR cur_buildings.boiler_installed_year <= boilerAgeMax) ELSE 1 END)
	AND (CASE WHEN boilerAgeMin IS NOT NULL THEN (cur_buildings.boiler_installed_year >= boilerAgeMin) ELSE 1 END)
	AND (CASE WHEN numOf1BedroomMax IS NOT NULL THEN (cur_buildings.bedroom1_units IS NULL OR cur_buildings.bedroom1_units <= numOf1BedroomMax) ELSE 1 END)
	AND (CASE WHEN numOf1BedroomMin IS NOT NULL THEN (cur_buildings.bedroom1_units >= numOf1BedroomMin) ELSE 1 END)
	AND (CASE WHEN numOf2BedroomMax IS NOT NULL THEN (cur_buildings.bedroom2_units IS NULL OR cur_buildings.bedroom2_units <= numOf2BedroomMax) ELSE 1 END)
	AND (CASE WHEN numOf2BedroomMin IS NOT NULL THEN (cur_buildings.bedroom2_units >= numOf2BedroomMin) ELSE 1 END)
	AND (CASE WHEN numOf3BedroomMax IS NOT NULL THEN (cur_buildings.bedroom3_units IS NULL OR cur_buildings.bedroom3_units <= numOf3BedroomMax) ELSE 1 END)
	AND (CASE WHEN numOf3BedroomMin IS NOT NULL THEN (cur_buildings.bedroom3_units >= numOf3BedroomMin) ELSE 1 END)
	AND (CASE WHEN numOfBachelorMax IS NOT NULL THEN (cur_buildings.bachelor_units IS NULL OR cur_buildings.bachelor_units <= numOfBachelorMax) ELSE 1 END)
	AND (CASE WHEN numOfBachelorMin IS NOT NULL THEN (cur_buildings.bachelor_units >= numOfBachelorMin) ELSE 1 END)
	AND (CASE WHEN priceOf1BedroomMax IS NOT NULL THEN (cur_buildings.bedroom1_price IS NULL OR cur_buildings.bedroom1_price <= priceOf1BedroomMax) ELSE 1 END)
	AND (CASE WHEN priceOf1BedroomMin IS NOT NULL THEN (cur_buildings.bedroom1_price >= priceOf1BedroomMin) ELSE 1 END)
	AND (CASE WHEN priceOf2BedroomMax IS NOT NULL THEN (cur_buildings.bedroom2_price IS NULL OR cur_buildings.bedroom2_price <= priceOf2BedroomMax) ELSE 1 END)
	AND (CASE WHEN priceOf2BedroomMin IS NOT NULL THEN (cur_buildings.bedroom2_price >= priceOf2BedroomMin) ELSE 1 END)
	AND (CASE WHEN priceOf3BedroomMax IS NOT NULL THEN (cur_buildings.bedroom3_price IS NULL OR cur_buildings.bedroom3_price <= priceOf3BedroomMax) ELSE 1 END)
	AND (CASE WHEN priceOf3BedroomMin IS NOT NULL THEN (cur_buildings.bedroom3_price >= priceOf3BedroomMin) ELSE 1 END)
	AND (CASE WHEN priceOfBachelorMax IS NOT NULL THEN (cur_buildings.bachelor_price IS NULL OR cur_buildings.bachelor_price <= priceOfBachelorMax) ELSE 1 END)
	...
	AND (CASE WHEN buildingIncomeMax IS NOT NULL THEN (cur_buildings.building_income IS NULL OR cur_buildings.building_income <= buildingIncomeMax) ELSE 1 END)
	AND (CASE WHEN unitPriceMin IS NOT NULL THEN (cur_buildings.unit_price >= unitPriceMin) ELSE 1 END)
	AND (CASE WHEN unitPriceMax IS NOT NULL THEN (cur_buildings.unit_price IS NULL OR cur_buildings.unit_price <= unitPriceMax) ELSE 1 END)
	AND (CASE WHEN lastSalesPriceMin IS NOT NULL THEN (cur_buildings.last_sale_price >= lastSalesPriceMin) ELSE 1 END)
	AND (CASE WHEN lastSalesPriceMax IS NOT NULL THEN (cur_buildings.last_sale_price IS NULL OR cur_buildings.last_sale_price <= lastSalesPriceMax) ELSE 1 END)
	AND (CASE WHEN numOfSalesMin IS NOT NULL THEN (sales_count.num_of_records >= numOfSalesMin) ELSE 1 END)
	AND (CASE WHEN numOfSalesMax IS NOT NULL THEN (sales_count.num_of_records IS NULL OR sales_count.num_of_records <= numOfSalesMax) ELSE 1 END)
	AND (CASE WHEN numOfParkingMin IS NOT NULL THEN (cur_buildings.parking_spots>= numOfParkingMin) ELSE 1 END)
	AND (CASE WHEN numOfParkingMax IS NOT NULL THEN (cur_buildings.parking_spots IS NULL OR cur_buildings.parking_spots <= numOfParkingMax) ELSE 1 END)
	AND (CASE WHEN windowInstallYearMax IS NOT NULL THEN (cur_buildings.windows_installed_year IS NULL OR cur_buildings.windows_installed_year <= windowInstallYearMax) ELSE 1 END)
