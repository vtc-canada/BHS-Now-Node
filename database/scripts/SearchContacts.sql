...
CREATE PROCEDURE `SearchContacts`(IN contactSearchTerms VARCHAR(128), IN addressSearchTerms VARCHAR(128)
...
FROM cur_contacts
	...
	INNER JOIN cur_company 
		ON (cur_company.id = cur_contact_company_mapping.cur_company_id))
	ON ( cur_contact_company_mapping.cur_contacts_id = cur_contacts.id)
